...
function getChromiumPath(): string | undefined {
  ...
  if (process.platform === 'win32') {
    ...
    for (const p of winPaths) {
      if (fs.existsSync(p)) {
        console.log(`[Events] Found Windows Chrome/Edge path: ${p}`);
        return p;
      }
    }
  }
  ...
  try {
      ...
      for (const bin of binNames) {
        ...
        if (fs.existsSync(fullPath)) {
          try {
            ...
            console.log(`[Events] Found Chromium in PATH: ${fullPath}`);
            return fullPath;
          } catch (e) {}
        }
      }
    ...
    console.error('[Events] Error scanning PATH:', err);
  }
  ...
  try {
    ...
    if (commandPath) {
      console.log(`[Events] Found Chromium via command -v: ${commandPath}`);
      return commandPath;
    }
  } catch (e) {}
  ...
  for (const p of commonPaths) {
    if (fs.existsSync(p)) {
      try {
        ...
        console.log(`[Events] Found Chromium in common paths: ${p}`);
        return p;
      } catch (e) {}
    }
  }

  console.warn('[Events] System Chromium not found, falling back to default.');
  return undefined;
}
...
`;
        ...
        try {
          ...
          console.error(`[Certificates] Failed to generate PDF for ticket ${ticket.id} via Puppeteer:`, pdfErr);
        }
